-- Bytelytic Clinic OS - Phase 8 Enterprise, White Label & EHR Integration Migrations
...
ALTER TABLE public.clinics ADD COLUMN IF NOT EXISTS group_id UUID REFERENCES public.clinic_groups(id) ON DELETE SET NULL;
ALTER TABLE public.clinics ADD COLUMN IF NOT EXISTS agency_id UUID REFERENCES public.agencies(id) ON DELETE SET NULL;
...
CREATE INDEX IF NOT EXISTS idx_global_identity_hashes ON public.global_patient_identity(group_id, phone_hash, dob_hash);

-- 5. Add global_identity_id and language_preference to patients table
ALTER TABLE public.patients ADD COLUMN IF NOT EXISTS global_identity_id UUID REFERENCES public.global_patient_identity(id) ON DELETE SET NULL;
ALTER TABLE public.patients ADD COLUMN IF NOT EXISTS language_preference VARCHAR(10) DEFAULT 'en';
...
-- 8. Create agent_configs table (with A/B testing & Retell synchronization columns)
...
-- 9. Add doctor info columns to clinics table
ALTER TABLE public.clinics ADD COLUMN IF NOT EXISTS npi_number TEXT;
ALTER TABLE public.clinics ADD COLUMN IF NOT EXISTS medical_license TEXT;
